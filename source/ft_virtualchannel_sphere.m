function [data_vc] = ft_virtualchannel_sphere(cfg, data, source, parcellation)
...
cfg.radius       = ft_getopt(cfg, 'radius', []); % in same unit as source.unit (e.g. mm)
...
if usepos
  ...
  % number of positions in the source and number of requested v-channels
  npos  = size(source.pos,1);
  nvc   = size(cfg.pos,1);

  % initialize index container: for each virtual channel we store indices of selected source gridpoints
  sel_idx = cell(nvc,1);
  mindist = zeros(nvc,1);

  % convert radius to source unit scale (cfg.radius is assumed in same unit as source.unit)
  if ~isempty(cfg.radius)
    if ~isnumeric(cfg.radius) || any(cfg.radius<=0)
      ft_error('cfg.radius should be a positive numeric scalar (in same units as source.unit)');
    end
  end

  for i = 1:nvc
    % distances from this center to all source grid positions
    dpos = sqrt(sum( (source.pos - cfg.pos(i.*ones(npos,1),:)).^2, 2));
    mindist(i) = min(dpos);
    if isempty(cfg.radius)
      % original behaviour: pick the closest source position and enforce <=1 mm tolerance
      [~, closest] = min(dpos);
      ...
      if isempty(source.filter{closest})
        ft_error(sprintf('the spatial filter information is missing for dipole with position [%d %d %d]', cfg.pos(i,:)));
      end
      sel_idx{i} = closest; % single index
    else
      % new behaviour: collect all source positions within cfg.radius
      sel = find(dpos <= cfg.radius);
      if isempty(sel)
        ft_error(sprintf('no source positions found within radius %g of position [%g %g %g]', cfg.radius, cfg.pos(i,:)));
      end
      % ensure each selected source has filter info
      emptyf = cellfun(@(x) isempty(x), source.filter(sel));
      if any(emptyf)
        badsel = sel(find(emptyf,1));
        ft_error(sprintf('the spatial filter information is missing for dipole at position [%g %g %g] (index %d)', source.pos(badsel,:), badsel));
      end
      sel_idx{i} = sel(:);
    end
  end

  % set a sensible balancing name for montage application
  bname = 'virtualchannel_sphere';
...
for i = 1:nvc
  ...
  montage = [];
 if usepos
  sel = sel_idx{i};                   % indices of source grid points (one or many)
  % concatenate the spatial filters for all selected gridpoints
  ...
  for k = 1:numel(sel)
    for kk = 1:size(source.filter{sel(k)},1)
      cnt = cnt + 1;
      montage.labelnew{cnt} = sprintf('virtualchannel%03d_dip%03d_orient%03d', i, sel(k), kk);
    end
  end
    warning('mes edit to allow radius of sphere')
  elseif useparcellation
...
if usepos
  ...
  brainordinate.index = zeros(size(source.pos,1), 1);

  % Each sphere may include several source grid points
  for i = 1:numel(sel_idx)
    brainordinate.index(sel_idx{i}) = i;
  end

  brainordinate.indexlabel = data_vc.label; % one label per virtual channel
elseif useparcellation
