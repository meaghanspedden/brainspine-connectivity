...
%construct filter for both conditions - use averaged data
cfg=[];
...
cfg=[]; %do this again because other didnt keep individ trials
cfg.output     = 'powandcsd';
cfg.method     = 'mtmfft';
cfg.foilim     = [10 35];
cfg.tapsmofrq  = 2;
...
%now apply filter to each condition (already computed filter so will apply
%to single trials with cfg.rawtrial)
tic
cfg=[];
cfg.grid = sources_combined;
cfg.headmodel=dummyvol;
...
spinepowrest=source_rest_avg.avg.pow(1:nspinesources);
spinepowcont=source_stat_avg.avg.pow(1:nspinesources);
figure; plot(sources_cent.pos(:,2),spinepowrest); hold on
plot(sources_cent.pos(:,2), spinepowcont)
