...
% Outputs saved with _BS suffix.
% Includes:
%   - Null distribution by orientation (P1 only)
%   - Dominant orientation extraction per subject
...
for ss = 1:length(subs_spine)
    ...
    if strcmp(sub, 'OP00212')
        ...
        F1 = filters{maxIdx_perm(1)};
fprintf('\n  Filter shape at peak perm source: [%d x %d]\n', size(F1,1), size(F1,2));
fprintf('  maxIdx_perm: min=%d  max=%d  n_unique=%d/%d\n', ...
    min(maxIdx_perm), max(maxIdx_perm), numel(unique(maxIdx_perm)), nPerm);
...
% paste diagnostic here
unique_srcs = unique(maxIdx_perm);
fprintf('\n  Dominant orientation per source:\n');
for k = 1:numel(unique_srcs)
    F = filters{unique_srcs(k)};
    [U,~,~] = svd(F, 'econ');
    dom = U(:,1);
    [~, ax] = max(abs(dom));
    fprintf('  Source %2d (y=%.1f mm): [%.3f  %.3f  %.3f]  axis=%d\n', ...
        unique_srcs(k), sources_cent.pos(unique_srcs(k),2), ...
        dom(1), dom(2), dom(3), ax);
end
...
bw      = 0.25;
offsets = [-bw, 0, bw];
F_test = coh_source.avg.filter{maxIdx_perm(1)};
fprintf('  Filter shape: %d x %d\n', size(F_test,1), size(F_test,2));
for ax_id = 1:3
    for s = 1:nSubjects
