%% RUN_SPECTRA_true_bem.m
% Downstream spectra and coherence analysis — corrected BEM forward model.
%
% Updated from RUN_SPECTRA_2.m:
%   - Brain VE from M1 ROI (brain-EMG coherence intersection) via step_A
%   - Spine VE from peak spine-EMG coherence location via step_3_VE_peak
%   - Updated paths to brainspine_savetest
%
% REQUIRES:
%   - sub{ID}_VE_brain_M1_brain_pct10.mat     (Step A)
%   - VE_spine_peak_sub{ID}_forspectra_bemv2_true_bem.mat  (Step 3 peak)
%
% STEPS:
...
% Coherence analysis: only subjects with BOTH VEs
cfg.subs_coh = intersect(cfg.subs_brain, cfg.subs_spine);

% VE configurations to compare
ve_configs = {
    'group_prevalence', 'VE_spine_sub%s_forspectra_bemv2%s.mat',        'VE';
    'peak_based',       'VE_spine_peak_sub%s_forspectra_bemv2%s.mat',   'VE';
    'anat_roi',         'VE_spine_anatROI_sub%s_forspectra_bemv2%s.mat','VE';
};
% VE filename suffixes
cfg.brain_ve_suffix = '_brain_pct10';   % from step_A_brain_VE_M1.m
cfg.spine_ve_suffix = '_true_bem';      % appended to all spine VE filenames
...
fprintf('\n=== SPECTRA CONFIG (true BEM) ===\n')
fprintf('  Brain subs: %d  |  Spine subs: %d  |  Coh subs: %d\n', ...
    numel(cfg.subs_brain), numel(cfg.subs_spine), numel(cfg.subs_coh))
fprintf('  Brain VE: sub{ID}_VE_brain_M1%s.mat\n', cfg.brain_ve_suffix)
fprintf('  Spine VE: VE_spine_peak_sub{ID}_forspectra_bemv2%s.mat\n', cfg.spine_ve_suffix)
fprintf('=================================\n\n')

fprintf('\n>>> STEP B: Coherence spectra\n')
for vi = 1:size(ve_configs,1)
    ve_label    = ve_configs{vi,1};
    cfg.spine_ve_pattern = ve_configs{vi,2};
    cfg.spine_ve_varname = ve_configs{vi,3};
    cfg.fig_suffix       = ['_true_bem_' ve_label];
    fprintf('\n>>> Running VE config: %s\n', ve_label);
    % Each VE config gets its own figures subfolder
    cfg.fig_dir = fullfile(cfg.save_dir, 'figures', ['spectra_' ve_label]);
...
for ss = 1:nSubs
    ...
    %% Load brain VE — M1 ROI from brain-EMG coherence intersection
...
sVE_file = fullfile(save_dir, ...
    sprintf(cfg.spine_ve_pattern, sub, cfg.spine_ve_suffix));
...
sVE      = sVE_data.(cfg.spine_ve_varname);
    ...
    %% FieldTrip coherence spectra (contraction)
    statB.label{1}   = 'brain';
    statS.label{1}   = 'spine';
    statEMG.label{1} = 'EMG';
    ...
    %% Individual subject plot (OP00212 only)
    if strcmp(sub, 'OP00212')
        ...
        xlabel('Frequency (Hz)','FontSize',14);
        ylabel('Coherence (x1e-3)','FontSize',14);
        title('Brain - Spine (true BEM)','FontSize',14);
        set(gca,'FontSize',14); box off;
        ...
        sgtitle(sprintf('Subject %s - non-zero lag coherence (true BEM)', sub), ...
            'Interpreter','none','FontSize',14);
        if saveFigs
            savefig(hfig_coh, fullfile(fig_dir, ...
                sprintf('sub%s_coherence_spectra_true_bem.fig', sub)));
        end

        %% Partial coherence figure P1
        fax_mt_sub = f1_mt(:,1);
        ...
        title(sprintf('Subject %s - Brain-EMG | spine (MT, true BEM)', sub), 'Interpreter','none','FontSize',14);
        ...
        if saveFigs
            savefig(hfig_part_p1, fullfile(fig_dir, ...
                sprintf('sub%s_partial_coherence_BE_spine_true_bem.fig', sub)));
        end
    end  % OP00212 only
...
%% Sub-band peak frequencies per pair
...
%% Partial coherence reduction by cluster
...
sgtitle('Brain-EMG coherence: full vs partial (spine removed) by cluster (true BEM)','FontSize',13);
if saveFigs
    savefig(hfig_part_clust, fullfile(fig_dir,'partial_coherence_by_cluster_true_bem.fig'));
end

%% Aligned partial coherence spectra
rel_win_p  = 12;
rel_res_p  = 0.25;
...
title('Brain-EMG: full vs partial coherence (aligned, true BEM)','FontSize',13);
set(gca,'FontSize',13); box off;
if saveFigs
    savefig(hfig_part_aligned, fullfile(fig_dir,'partial_coherence_aligned_true_bem.fig'));
end
...
sgtitle('Peak frequency tracking across signal pairs (GMM clusters, true BEM)','FontSize',13);
if saveFigs
    savefig(hfig_bp, fullfile(fig_dir,'group_subband_peak_frequencies_true_bem.fig'));
end
...
sgtitle('Peak frequency clustering (GMM, true BEM)','FontSize',13);
if saveFigs
    savefig(hfig_clust, fullfile(fig_dir,'group_peak_frequency_clustering_true_bem.fig'));
end
...
pair_names_g = {'Brain-EMG', 'Brain-Spine (true BEM)', 'Spine-EMG'};
...
sgtitle('Group coherence spectra aligned to individual beta peak (true BEM)','FontSize',13);
if saveFigs
    savefig(hfig_aligned, fullfile(fig_dir,'group_coherence_spectra_aligned_true_bem.fig'));
end
...
if sum(okBf) > 1
    ...
    fprintf('  Brain (periodic): insufficient data\n');
end
if sum(okSf) > 1
    ...
    fprintf('  Spine (periodic): insufficient data\n');
end
...
disp(supp_table_1a);
csv_path_1a = fullfile(fig_dir, 'SuppTable1a_peak_frequency_true_bem.csv');
try
    writetable(supp_table_1a, csv_path_1a, 'WriteRowNames', true);
    fprintf('  Saved: %s\n', csv_path_1a);
catch ME
    warning('Could not save Table 1a: %s', ME.message);
end
...
fprintf('\n  Supplementary Table 1b: Peak coherence value (R2)\n');
disp(supp_table_1b);
csv_path_1b = fullfile(fig_dir, 'SuppTable1b_peak_coherence_true_bem.csv');
try
    writetable(supp_table_1b, csv_path_1b, 'WriteRowNames', true);
    fprintf('  Saved: %s\n', csv_path_1b);
catch ME
    warning('Could not save Table 1b: %s', ME.message);
end
...
if n_complete >= 3
    ...
    for c = 1:size(combos,1)
        ...
        fprintf('    %s vs %s: p=%.4g [%s]\n', pair_names{a}, pair_names{b}, ...
            p_posthoc(c), conditional_str(sig,'significant','ns'));
    end
end
...
title('Group directed coherence (true BEM)','Interpreter','none','FontSize',14);
if saveFigs
    savefig(hfig_bar, fullfile(fig_dir, 'group_directed_coherence_true_bem.fig'));
end

%% Group: directionality comparison
...
sgtitle('Directionality: Halliday R2 (top) vs PSI (bottom, true BEM)','FontSize',14,'Interpreter','none');
if saveFigs
    savefig(hfig_dir_comp, fullfile(fig_dir, 'group_directionality_comparison_true_bem.fig'));
end
...
title('Cross-correlation peak latencies (true BEM)','Interpreter','none','FontSize',14);
if saveFigs
    savefig(hfig_lat, fullfile(fig_dir, 'group_peak_latencies_true_bem.fig'));
end
