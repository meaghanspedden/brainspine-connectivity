...
cfg_pipeline.subs_spine     = {'OP00212'};   % subject 1 only
cfg_pipeline.saveFigs       = 0;
geoms_path = '';   % path to shifted leadfield folders
...
%% =========================================================================
%  SENSITIVITY ANALYSIS — set run_step1-4 all to 0 before running this
%% =========================================================================

run_sensitivity_coherence(geoms_path, cfg_pipeline);
...
function run_sensitivity_coherence(geoms_path, cfg_pipeline)

% find all geometry folders in geoms_path
d     = dir(fullfile(geoms_path, 'geometries_*'));
dirs  = {d([d.isdir]).name};   % keep directories only
dirs  = [{'geometries_original'} setdiff(dirs, 'geometries_original')];  % original first

set(0, 'DefaultFigureVisible', 'off');

for ci = 1:numel(dirs)
    folder = dirs{ci};
    label  = regexprep(folder, '^geometries_', '');   % strip prefix for filenames

    % Find leadfield file inside this folder
    lf_files = dir(fullfile(geoms_path, folder, 'leadfield_*_bem_experimental.mat'));
    if isempty(lf_files)
    ...
    lf_file   = fullfile(geoms_path, folder, lf_files(1).name);
    ...
    save_dir = fullfile(geoms_path, 'sensitivity_coherence', label);
