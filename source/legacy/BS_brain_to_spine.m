%% step_B_M1VE_spine_coherence_bslaw.m
% DICS coherence between M1 virtual electrode and spinal cord sources.
% Uses BSLaw leadfield for spine forward model.
% Contraction vs rest permutation test, 20mm smoothing.
...
fig_dir = fullfile(save_dir, 'figures', 'stepB_bslaw');
...
for ss = 1:numel(subs_both)
    ...
    %% Build BSLaw spine leadfield — label-based matching
    ...
    %% Append M1 VE to spine data
    trialinfo          = ftdat.trialinfo;
    spinedat           = ft_appenddata([], ftdat, VE_brain);
    ...
    [~, peak_idx] = max(coh_diff);
    fprintf('  Peak at y=%.1f mm (source %d)\n', cord_pos(peak_idx), peak_idx);
    ...
    title(sprintf('%s — M1VE-spine coherence (BSLaw)', sub),'Interpreter','none');
    ...
    title(sprintf('%s — null maxima (M1VE-spine BSLaw)', sub),'Interpreter','none');
...
fprintf('  %d/%d subjects significant (M1VE-spine BSLaw)\n', sum(sig_pos), nSubjects);
...
title('M1VE-spine coherence (BSLaw, smoothed 20mm)');
...
if yl_clust(2) == 0, yl_clust(2) = 0.1; end
fill([cord_pos(roi_idx(1))   cord_pos(roi_idx(end)) ...
      cord_pos(roi_idx(end)) cord_pos(roi_idx(1))], ...
...
title('Group prevalence — M1VE-spine coherence (BSLaw)','Interpreter','none');
