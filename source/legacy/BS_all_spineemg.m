...
for ss = 1:length(subs_spine)
    ...
    %% Null maxima orientation diagnostic — all three components
[~, maxIdx_perm] = max(cohDiff_perm, [], 1);   % ADD THIS LINE
...
% Figure
hfig_ori = figure('Color','w','Position',[100 100 500 430]);
hold on;
...
for ax_id = 1:3
    ...
    % Violin-style: kernel density
    [f, xi] = ksdensity(vals, 'Support',[0 1]);
    f_norm  = f / max(f) * 0.35;   % half-width of violin
    xc      = ax_id;

    fill([xc - f_norm, fliplr(xc + f_norm)], [xi, fliplr(xi)], ...
        ax_colors(ax_id,:), 'FaceAlpha', 0.4, 'EdgeColor','none');
    plot(xc - f_norm, xi, '-', 'Color', ax_colors(ax_id,:), 'LineWidth', 0.8);
    plot(xc + f_norm, xi, '-', 'Color', ax_colors(ax_id,:), 'LineWidth', 0.8);

    % Median line
    med_val = median(vals);
    [~, i_med] = min(abs(xi - med_val));
    plot([xc - f_norm(i_med), xc + f_norm(i_med)], [med_val med_val], ...
        '-', 'Color', ax_colors(ax_id,:), 'LineWidth', 2.0);

    % Observed value
    scatter(xc, obs_comps(ax_id), 100, 'k', 'filled', ...
        'MarkerEdgeColor','w', 'LineWidth', 1.2);
end

% Legend
h_vio = fill(nan, nan, [0.6 0.6 0.6], 'FaceAlpha',0.4, 'EdgeColor','none');
h_obs = scatter(nan, nan, 100, 'k', 'filled', 'MarkerEdgeColor','w');
legend([h_vio h_obs], {'Null distribution','Observed'}, ...
...
title(sprintf('%s — source orientation (BS)', sub), ...
