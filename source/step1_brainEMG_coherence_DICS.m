...
% Step 1: Brain-EMG DICS coherence, all 7 brain subjects.
%
% Computes DICS beamformer coherence between brain sources and rectified
% EMG (10-35 Hz), tests contraction vs rest via label permutation, and
% produces per-subject and group-level (prevalence) coherence maps.
%
% Regularisation: lambda = '10%' (matches the spinal cord analysis).
%
% Inputs (must exist before running):
%   geomfile   - .mat containing sources_brain, mesh_brain, mesh_torso etc.
%   T_mat_path - .mat containing T (MNI transform matrix)
%   data_root  - per-subject SPM .mat files (see datafile pattern below)
%
% Outputs (written to save_dir / fig_dir):
%   brainResult_sub<ID>_brain_pct10.mat   - per-subject coh_diff, thr95, mask, invp, x_mni
%   groupRes_brain_DICS_brain_pct10.mat   - subjResults struct for all subjects
%   step1_sub<ID>_brainEMG_coherence_brain_pct10.fig - per-subject coherence map
%   step1_group_brain_max_brain_pct10.fig            - group peak location
%   step1_group_brain_prevalence_brain_pct10.fig     - group prevalence map
%
