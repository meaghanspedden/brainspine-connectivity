%% step2_spineEMG_coherence_DICS_v2_leadfieldChoice.m
% Step 2 (v2, leadfield choice): Spine-EMG DICS coherence, with a choice of
% forward model:
%   leadfield_type = 'BS'       - precomputed Biot-Savart leadfield, loaded
%                                  from lf_path and channel-matched (identical
%                                  to step2_spineEMG_coherence_DICS_v2.m)
%   leadfield_type = 'infinite' - leadfield computed fresh per subject via
%                                  ft_prepare_leadfield against the infinite
%                                  homogeneous volume conductor (the same
%                                  headmodel already used as the dummy
%                                  headmodel passed to ft_sourceanalysis)
%
% Everything else (frequency analysis, DICS, permutation test, smoothing,
% thresholding, plotting, group analysis, ROI clustering) is unchanged
% between the two options. out_suffix and figure/log labels are derived
% automatically from leadfield_type so the two runs never overwrite each
% other's outputs.
%
% Inputs (must exist before running):
%   geomfile       - geometries_experimental_withbrain.mat: sources_cent,
%                     mesh_torso, mesh_wm, mesh_bone, mesh_lungs, mesh_heart,
%                     mesh_brain
%   lf_path        - leadfield_experimental_bslaw_experimental.mat: leadfield_bs
%                     (only needed when leadfield_type = 'BS')
%   data_root      - per-subject SPM .mat files (see datafile pattern below)
%
% Outputs (written to save_dir / fig_dir), suffixed with out_suffix
% ('_BS' or '_infinite' depending on leadfield_type):
%   subResult_sub<ID><out_suffix>.mat
%   groupRes_spine_DICS_<out_suffix>.mat
%   cluster_spineEMG_pos<out_suffix>.mat
%   step2_sub<ID>_spineEMG_coherence<out_suffix>.fig
%   step2_sub1_spineEMG_meshonly<out_suffix>.fig
%   step2_sub<ID>_null_maxima<out_suffix>.fig
%   step2_subOP00212_null_distribution_by_orientation<out_suffix>.fig
%   step2_group_spineEMG_prevalence<out_suffix>.fig
%   step2_group_spineEMG_prevalence_meshonly<out_suffix>.fig
%   step2_group_spineEMG_subject_lines<out_suffix>.fig
%   step2_group_dominant_orientation<out_suffix>.fig / .png
...
save_dir       = 'C:\Users\mspedden\Documents\brainspine_savetest11';
...
leadfield_type = 'BS';   % 'BS' or 'infinite' — choose leadfield source
...
switch leadfield_type
    case 'BS'
        out_suffix = '_BS';
        lf_label   = 'BS';
    case 'infinite'
        out_suffix = '_infinite';
        lf_label   = 'Infinite';
    otherwise
        error('leadfield_type must be ''BS'' or ''infinite''.');
end
...
for ss = 1:length(subs_spine)
    ...
    %% Volume conductor (infinite homogeneous — used as dummy headmodel for
    %  DICS/permutation regardless of leadfield_type, and as the actual
    %  forward-model headmodel when leadfield_type = 'infinite')
    cfg = []; cfg.method = 'infinite'; cfg.siunits = 1;
    cfg.grad = grad_mm; cfg.conductivity = 1;
    dummyvol = ft_prepare_headmodel(cfg, mesh_torso);

    %% Build leadfield — label-based matching (shared by both leadfield types)
    fprintf('  Building leadfield (%s)...\n', lf_label);
    ...
    switch leadfield_type
        ...
        case 'infinite'
            [common_labels, ~] = intersect(grad_mm.label, data_meg_labels, 'stable');
            fprintf('  Data MEG: %d  |  Grad: %d  |  Matched: %d\n', ...
                numel(data_meg_labels), numel(grad_mm.label), numel(common_labels));
            if numel(common_labels) < numel(data_meg_labels)
                fprintf('  WARNING: %d channels not in grad\n', ...
                    numel(data_meg_labels) - numel(common_labels));
            end

            cfg_lf                  = [];
            cfg_lf.grad             = grad_mm;
            cfg_lf.headmodel        = dummyvol;
            cfg_lf.sourcemodel.pos  = sources_cent.pos;
            cfg_lf.sourcemodel.unit = sources_cent.unit;
            cfg_lf.channel          = common_labels;
            cfg_lf.reducerank       = 'no';
            cfg_lf.normalize        = 'no';
            lf_ft = ft_prepare_leadfield(cfg_lf);

            Lf           = [];
            Lf.label     = common_labels;
            Lf.pos       = sources_cent.pos;
            Lf.inside    = ones(nsourcepoints, 1);
            Lf.leadfield = lf_ft.leadfield;
    end
    ...
    title(sprintf('%s — null maxima (%s)', sub, lf_label),'Interpreter','none');
    ...
    if strcmp(sub, 'OP00212')
        ...
        legend off;
        title(sprintf('%s — null distribution by orientation (smoothed %d mm, %s)', ...
            sub, fwhm_mm, lf_label),'Interpreter','none','FontSize',13);
    ...
    title(sprintf('%s — spine-EMG coherence (%s)', sub, lf_label),'Interpreter','none');
    ...
    if ss == 1
        ...
        colorbar off;
        view(-250,-1); camlight; ax = gca; ax.FontSize = 14;
        title(sprintf('Participant 1 — spine-EMG coherence (%s)', lf_label), ...
...
fprintf('  %d/%d subjects show significant spine-EMG coherence (smoothed %d mm, %s)\n', ...
    sum(sig_pos), nSubjects, fwhm, lf_label);
...
ft_plot_mesh(mesh_lungs,'facecolor',[0.8 0.3 0.3],'facealpha',0.1, 'edgecolor','none');
ft_plot_mesh(mesh_heart,'facecolor',[0.8 0.3 0.3],'facealpha',0.1, 'edgecolor','none');
title(sprintf('Group prevalence — spine-EMG (smoothed %d mm, %s)', fwhm, lf_label), ...
    'Interpreter','none');
...
title(sprintf('Spine-EMG coherence differences (smoothed %d mm, %s)', fwhm, lf_label));
...
if ~isempty(pos_thresh)
    ...
    title(sprintf('Spinal prevalence + VE cluster (%s)', lf_label),'Interpreter','none');
