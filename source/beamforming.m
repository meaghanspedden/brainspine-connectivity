...
%D=spm_opm_attach_leadfield(leadfield.label,G,1,D)
%[L, gainmatnames, lflabels] = read_lf(rootlf, Ncordpoints, castforward);


% PREP SENSOR CHANNEL INFO
chanind = Drest.indchantype('MEG');
channames = Drest.chanlabels(chanind);
sensors = Drest.sensors('MEG');
...
for nn = 1% (runs)
    Dbf1 = Drest;
    Dbf2 = Dstatic;
    chanind = indchantype(Dbf1, 'MEGMAG');
    chanind = chanind(~ismember(chanind, Dbf1.badchannels));
    channames = Dbf1.chanlabels(chanind);
    sensors = Dbf1.sensors('MEG');
    ...
    covsamples = find(Dbf1.time > covtime(1) & Dbf1.time < covtime(2));
    Ntrials = Dbf1.ntrials+Dbf2.ntrials;
    allDbf=cat(3,Dbf1(:,:,:),Dbf2(:,:,:));
    ...
    lfbodyind = find(ismember(leadfield.label, Dbf1.chanlabels(bodyind)));
    ...
    wdatarest=wdata(:,1:Dbf1.ntrials,:,:);
    wdatastatic=wdata(:,Dbf1.ntrials+1:end,:,:);

    % Save results
    bffilename = fullfile(save_dir, ['bfdata_', sub]);
    spmfilename = Dbf1.fullfile;
...
%load and run the beamformer data
% load_bfdata_maike(allbfnames);
