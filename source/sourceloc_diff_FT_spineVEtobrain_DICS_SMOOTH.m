%% DICS spinal VE to brain (with spatial smoothing applied to OBSERVED + PERMUTATIONS)
% -----------------------------------------------------------------------------
% Changes vs your original:
%  1) New save_dir (no overwrite)
%  2) Build Gaussian smoother Wsm on INSIDE brain grid points (sources_brain.pos)
...
smooth_fwhm_mm   = 20;                 % <-- brain smoothing FWHM
smooth_radius_mm = 3*smooth_fwhm_mm;    % truncate kernel; 3*FWHM is robust for 20 mm

save_dir = fullfile(base_save_dir, sprintf('brain_permSmooth_%dmm', smooth_fwhm_mm));
...
for ss=1:length(subs)
    ...
    if rectify
        ...
        for k=1:length(ftdat.trial)
            ftdat.trial{k}(end,:)=ftdatr.trial{k}; % replace EXG1 row
        end
    end
    ...
    %% Append VE + preserve trialinfo
    braindat = ftdat;
    loadfile = sprintf('VE_spine_sub%s_forspectra_newLFVE_spine_subVE_forspectra_newLF.mat', sub);
    load(fullfile(base_save_dir, loadfile))  % NOTE: VE saved in your base folder (adjust if needed)
    ...
    %% Leadfield
    cfg                     = [];
    cfg.sourcemodel         = sources_brain;  % must exist in geomfile
    cfg.headmodel           = vol;
    cfg.grad                = braingrad;
    cfg.reducerank          = 'no';
    cfg.normalize           = 'no';
    ...
    %% DICS filter from both conditions
    cfg=[];
    ...
    %% Apply filters and run permutation
    cfg=[];
    cfg.grid = sources_brain;
    cfg.headmodel = vol;
    cfg.sourcemodel.leadfield = LF;
    ...
    % For reproducibility independent of later RNG usage:
    rng(1);
    source_perm = ft_sourceanalysis(cfg, statdat, restdat);

    % -------------------- Inside indexing (IMPORTANT) ----------------------
    % FieldTrip source structs typically have:
    %   source_perm.pos    [N x 3]
    %   source_perm.inside [N x 1] logical
    inside = source_perm.inside(:);
    inside_idx = find(inside);
    nInside = numel(inside_idx);

    % Observed diff over ALL points
    coh_diff_all = source_perm.avgA.coh - source_perm.avgB.coh;
    coh_diff_in  = coh_diff_all(inside_idx);

    % Permutation diffs over INSIDE points only
    ...
    % -------------------- NEW: build spatial smoother on INSIDE positions ---
    pos_in = source_perm.pos(inside_idx, :);  % mm
    Wsm = make_gaussian_smoother(pos_in, smooth_fwhm_mm, smooth_radius_mm);

    cohDiff_perm_sm_in = Wsm * cohDiff_perm_in;  % [nInside x nPerm]
    coh_diff_sm_in     = Wsm * coh_diff_in;      % [nInside x 1]
    % ----------------------------------------------------------------------

    % Max-stat threshold from SMOOTHED permutations (inside only)
    maxPerm = max(cohDiff_perm_sm_in, [], 1);  % 1 x nPerm

    if mult_comp_corr
        thr95 = prctile(maxPerm, 95);      % scalar family-wise threshold
    else
    ...
    % One-sided p-values using SMOOTHED distributions
    ...
    %% Build source structs for plotting
    source_p = coh_source;
    ...
    if isempty(sigvals)
        cfg.funcolorlim = [invpthr invpthr+1]; % safe
    else
    ...
    %% Get max location (use SMOOTHED coh-diff used for inference)
    % If you want max significant location:
    if any(mask_in)
    ...
    maxcohindx_all = inside_idx(imax_in);
    maxpos = source_perm.pos(maxcohindx_all, :);

    % MNI transform (as you had)
    ...
    title(sprintf('Max (smoothed) @ [%0.1f %0.1f %0.1f] mm; MNI [%0.1f %0.1f %0.1f]', ...
        maxpos(1),maxpos(2),maxpos(3), x_mni(1),x_mni(2),x_mni(3)));

    %% Save subject results
    subjResults(ss).subjID  = sub;
    subjResults(ss).coh_diff = coh_diff_sm_all;     % CHANGED: smoothed observed map used for inference
    subjResults(ss).thr95    = thr95;
    subjResults(ss).sig_mask = mask_all;            % binary over grid (0/1)
    subjResults(ss).pos      = source_perm.pos;
    subjResults(ss).inside   = source_perm.inside;

    subjResults(ss).maxdiff.idx_all = maxcohindx_all;
    subjResults(ss).maxdiff.pos_native = maxpos;
    subjResults(ss).maxdiff.pos_mni = x_mni;

end

save(fullfile(save_dir,'groupRes_brain_DICS_spineVC_permSmooth.mat'), 'subjResults')

%% ------------------ GROUP ANALYSIS (prevalence) --------------------------
nSubjects = numel(subjResults);
sig_res = false(nSubjects,1);
for i = 1:nSubjects
    sig_res(i) = any(subjResults(i).sig_mask(:) > 0);
end
fprintf('%g/%g subjects show sig coherence anywhere (smoothed inference)\n', sum(sig_res), nSubjects)

all_masks = cat(2, subjResults(:).sig_mask);
group_prevalence = mean(all_masks, 2); % over subjects (grid)
...
%% Report MNI for max prevalence location
% (use same T_inv from earlier; reload if needed)
load('C:\Users\mspedden\Documents\brainspine_save\T.mat');
T_inv = inv(T);

[maxval, maxidx] = max(group_ft.pow);
maxpos = group_ft.pos(maxidx,:);
maxpos_h = [maxpos, 1]';
x_mni_h = T_inv * maxpos_h;
x_mni = x_mni_h(1:3,:)';

fprintf('Group max prevalence at native [%0.1f %0.1f %0.1f], MNI [%0.1f %0.1f %0.1f]\n', ...
    maxpos(1),maxpos(2),maxpos(3), x_mni(1),x_mni(2),x_mni(3));
...
% radius_mm: truncate kernel beyond this radius (recommend >= 3*sigma or ~3*FWHM)
