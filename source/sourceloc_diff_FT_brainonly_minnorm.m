...
LF = ft_prepare_leadfield(cfg);
    ...
    cfg=[];
    cfg.trials=statidx;
    statdat=ft_selectdata(cfg,braindat); %trial wise data pr condition

    cfg.trials=restidx;
    restdat=ft_selectdata(cfg,braindat);
    ...
    cfg.foilim     = [10 35];
    cfg.tapsmofrq  = 1;
    freqdat_stat=ft_freqanalysis(cfg,statdat); %trial wise freq
    freqdat_rest=ft_freqanalysis(cfg,restdat); %trial wise freq


cfg = [];
cfg.method      = 'eloreta';
cfg.sourcemodel = sources_brain;    % your source grid
cfg.headmodel   = vol;              % single-shell brain head model
cfg.grad        = braingrad;        % MEG grad info
cfg.mne.lambda  = 5;                % regularization factor (try 3-5%)
%cfg.mne.scalesourcecov = 'yes';     % normalize by source variance
%cfg.keeptrials  = 'yes';            % to compute stats later
cfg.channel     = brainlabs;        % use brain channels only

source_stat_mne = ft_sourceanalysis(cfg, freqdat_stat);

source_rest_mne = ft_sourceanalysis(cfg, freqdat_rest);

diff_source=source_stat_mne;
diff_source.avg.pow = ((source_stat_mne.avg.pow - source_rest_mne.avg.pow) ./ source_rest_mne.avg.pow) * 100;
if ndims(diff_source.avg.pow) > 1 && size(diff_source.avg.pow, 2) > 1
    diff_source.avg.pow = mean(diff_source.avg.pow, 2);
end
...
source_int = ft_sourceinterpolate(cfg, diff_source, mesh_brain);
