function  CVA_coh(Ydat,Xdat,usefreq,whichanalysis,subjectID)


%function [chi_lin, chi_env, chi_cross, df_lin, df_env, df_cross]= CVA_coh(Ydat,Xdat,refdat,whichanalysis,subjectID)
...
for f=1:numel(usefreq)
    ...
    CVA=spm_cva(Y,X); %I think over trials?
...
%% now run p vals from per frequency analyses through FDR
    ...
    [~, ~, ~, adj_p]=fdr_bh(p_lin,0.05,'dep','no');
    linSig=find(adj_p<0.05);

    [~, ~, ~, adj_p]=fdr_bh(p_env,0.05,'dep','no');
...
%     savename=sprintf('PA_sub%s_%s_%g.pdf',sub,whichanalysis,cnd);
